...
export function IntelligencePanel({ address, equity, mode, running }: Props) {
  ...
  return <section className="ledger-surface border border-[#6e5930] p-4 shadow-[0_18px_50px_rgba(0,0,0,.18)] sm:p-5">
    <div className="flex flex-col gap-3 border-b border-[#1b3b2c] pb-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-[9px] uppercase tracking-[.24em] text-[#00E676]">AURA intelligence register</p><h2 className="mt-1 font-display text-xl text-[#f4ecdb]">Helius telemetry <span className="text-[#8ba99a]">·</span> Groq copilot</h2></div><button onClick={refreshHelius} disabled={!address || heliusState === "loading"} className="rounded-sm border border-[#80652c] px-3 py-2 font-mono text-[9px] uppercase tracking-[.14em] text-[#f2cf7a] transition hover:bg-[#3b3021] disabled:opacity-50">{heliusState === "loading" ? "Scanning…" : "Scan wallet"}</button></div>
    <div className="grid gap-4 pt-4 lg:grid-cols-[1fr_1.3fr]"><div className="border border-[#1b3b2c] bg-[#211d16]/70 p-4"><p className="font-mono text-[9px] uppercase tracking-[.16em] text-[#8ba99a]">Helius live feed</p><p className="mt-3 text-sm text-[#f0e3c2]">{walletFlow}</p><p className="mt-2 text-xs leading-5 text-[#8ba99a]">Wallet activity and token discovery inform the desk. Telemetry alone never places a trade.</p><span className={`mt-4 inline-flex border px-2 py-1 font-mono text-[9px] ${heliusState === "ready" ? "border-[#00E676] text-[#00E676]" : heliusState === "error" ? "border-[#9945FF] text-[#9945FF]" : "border-[#80652c] text-[#00E676]"}`}>{heliusState === "ready" ? "CONNECTED" : heliusState === "error" ? "DEGRADED" : "READY TO SCAN"}</span></div><div className="border border-[#1b3b2c] bg-[#211d16]/70 p-4"><p className="font-mono text-[9px] uppercase tracking-[.16em] text-[#8ba99a]">Groq copilot</p><p className="mt-3 min-h-12 text-sm leading-6 text-[#f0e3c2]">{answer}</p><form onSubmit={askCopilot} className="mt-4 flex gap-2"><input value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Why is this signal safe?" className="min-w-0 flex-1 rounded-sm border border-[#6e5930] bg-[#15130f] px-3 py-2 text-sm text-[#f4ecdb] outline-none placeholder:text-[#8f8168] focus:border-[#00E676]" /><button disabled={loading} className="rounded-sm bg-[#00E676] px-3 py-2 font-mono text-[9px] font-bold uppercase tracking-[.12em] text-[#21180b] disabled:opacity-50">{loading ? "…" : "Ask"}</button></form></div></div>
  </section>;
}
